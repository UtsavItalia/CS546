...
const checkServiceOptions = (serviceOptions) => {
    ...
    for (const key in serviceOptions) {
        if (
            ...
            Object.keys(serviceOptions).length != 3
        ) {
            throw new Error(
                `You must add only dineIn, takeOut and delivery as serviceOptions`,
            );
        }
...
const create = async (
    ...
    checkifStringOrEmpty(name, location, phoneNumber, website, priceRange);
...
const update = async (
    ...
    if (typeof id !== "string") {
        throw new Error(`${id} you have entered is not a string`);
    }
    checkifStringOrEmpty(name, location, phoneNumber, website, priceRange);
    checkPhoneNumber(phoneNumber);
    checkWebsite(website);
